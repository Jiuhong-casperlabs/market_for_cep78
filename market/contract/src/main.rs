...
use casper_types::{
    ...
    runtime_args, CLTyped, ContractHash, Key, Parameter, RuntimeArgs, URef, U256, U512,
};
...
pub extern "C" fn create_listing() -> () {
    ...
    if token_owner != get_token_owner(token_contract_hash, &token_id) {
...
pub fn buy_listing() -> () {
    ...
    runtime::call_contract::<()>(
        ...
        runtime_args! {
            "recipient" => seller,
            "owner" => owner,
            "amount" => U256::from(listing.price.as_u64())
        },
    );
...
pub extern "C" fn make_offer() -> () {
    ...
    offers.insert(bidder, cep18_amount);
