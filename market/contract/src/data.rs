...
use casper_types::{
    bytesrepr::ToBytes, runtime_args, system::CallStackElement, ApiError, CLTyped, ContractHash,
...
pub fn contract_package_hash() -> ContractHash {
...
pub fn transfer_approved(token_contract_hash: ContractHash, token_id: &str, owner: Key) -> bool {
    let approved = runtime::call_contract::<Option<Key>>(
        ...
        runtime_args! {
          "owner" => owner,
          "token_id" => u64::from_str(&token_id).unwrap()
        },
    );
...
pub fn get_token_owner(token_contract_hash: ContractHash, token_id: &str) -> Key {
...
pub fn get_listing(listing_id: &str) -> (Listing, URef) {
    ...
    let listing: Listing = match storage::dictionary_get(dictionary_uref, &listing_id) {
        Ok(item) => match item {
